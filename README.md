...
| `dm1q-S9110ZCS8FZH3` | Galaxy S23 (base, China) `SM-S9110` | `5.15.189` | Built and statically verified; hardware run pending |
...
The S9110 ZCS8FZH3 profile covers the China Galaxy S23 base model. Its `.text`
is identical to the S916B FZG1 and S918B FZF5 builds, but four data-segment
offsets are `0x5C0` lower and were re-derived from this firmware's own
`boot.img`, so neither neighbouring payload can be reused. The payload is a
bionic shared library like the others and its KernelSU handoff is audited
against the device's kallsyms, but the profile has not been executed on
hardware yet. See [`docs/SM-S9110-S9110ZCS8FZH3.md`](docs/SM-S9110-S9110ZCS8FZH3.md).
...
make TARGET=dm1q-S9110ZCS8FZH3 ANDROID_NDK_HOME=/path/to/android-ndk
```
...
The China Galaxy S23 base model (`SM-S9110`) ZCS8FZH3 port is in
